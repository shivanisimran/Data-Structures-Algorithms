...
Solution:                                                                                                                                     */
   vector<vector<int>> floodFill(vector<vector<int>>& im, int r, int c, int ne) {
        int x=im[r][c];
    ...
    void dfs(vector<vector<int>>&im,vector<vector<int>>&vis,int x,int r,int c,int ne){
        if((r>=0 && r<im.size()) && (c>=0 && c<im[0].size()) && im[r][c]==x && !vis[r][c]){
            vis[r][c]=1;
            im[r][c]=ne;
            dfs(im,vis,x,r-1,c,ne);
            dfs(im,vis,x,r+1,c,ne);
            dfs(im,vis,x,r,c-1,ne);
            dfs(im,vis,x,r,c+1,ne);
        }
        else  return;
    }
