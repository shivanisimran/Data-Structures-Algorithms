...
Find the coordinates of the top left and bottom right corner of each group of farmland. A group of farmland with a 
top left corner at (r1, c1) and a bottom right corner at (r2, c2) is represented by the 4-length array [r1, c1, r2, c2].
...
Solution:                                                                                                                           */
    ...
    void dfs(int x,int y,vector<vector<int>>& vis,vector<vector<int>>&g,int n,int m,int& p,int& q){
        if((x>=0 && x<n) && (y>=0 && y<m) && !vis[x][y] && g[x][y]){
             vis[x][y]=1;
             if(x>p)    p=x;
             if(y>q)    q=y;
             dfs(x+1,y,vis,g,n,m,p,q);
             dfs(x-1,y,vis,g,n,m,p,q);
             dfs(x,y+1,vis,g,n,m,p,q);
             dfs(x,y-1,vis,g,n,m,p,q);
        }
       else return;
