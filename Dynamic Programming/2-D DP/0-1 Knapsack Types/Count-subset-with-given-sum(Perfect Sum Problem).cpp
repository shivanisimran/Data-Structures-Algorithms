...
Solution:                                                                                                                                              */
	int mod=1000000007;
	int knp(int arr[], int n, int s, vector<vector<int>>& dp ){
	    if(s==0) return 1;
	    if(n==0) return 0;
	    
	    if(dp[n][s]!=-1)  return dp[n][s];
	    
	    if(arr[n-1]<=s) return dp[n][s]=(knp(arr,n-1,s-arr[n-1],dp)+knp(arr,n-1,s,dp))%mod;
	    else return dp[n][s]=knp(arr,n-1,s,dp)%mod;
	}
	...
	int perfectSum(int arr[], int n, int sum)
	{
	       ...
	       return (knp(arr,n,sum,dp)+(1 << z)-1)%mod;
	}
